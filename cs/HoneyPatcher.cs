...
{	
	...
	private void DDSFixHeader(){
		string[] ddsList = Directory.EnumerateFiles(usrdir, "*d5comp*.dds", SearchOption.AllDirectories).ToArray();
		var badger = new HoneyBadger();
		if (!badger.DDSFixHeader(ddsList, true))
			HoneyLog(2, "An invalid dds header was found. Skipping.");
		ddsList = Directory.EnumerateFiles(usrdir, "*nocomp*.dds", SearchOption.AllDirectories).ToArray();
		if (!badger.DDSFixHeader(ddsList, false))
			HoneyLog(2, "An invalid dds header was found. Skipping.");
